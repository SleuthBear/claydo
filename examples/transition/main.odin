...
headerTextConfig := claydo.Text_Element_Config{font_id = 1, spacing = 5, font_size = 16, text_color = {0,0,0,255}}

frameArena: claydo.Arena

HandleHeaderButtonInteraction :: proc(elementId: claydo.Element_ID, cursor_data: claydo.Cursor_Data, userData: rawptr) {
...
HeaderButtonStyle :: proc(hovered: bool) -> claydo.Element_Declaration
{
        return {
                ...
                color = hovered ? COLOR_ORANGE : COLOR_BLUE,
    };
}
...
dropdownTextItemLayout := claydo.Layout_Config{padding = {8, 8, 4, 4} };
dropdownTextElementConfig := claydo.Text_Element_Config{font_size = 24, text_color = {1,1,1,1}  };
...
GG := claydo.Sizing{{.GROW, claydo.Sizing_Min_Max{0, claydo.MAX_FLOAT}}, {.GROW, claydo.Sizing_Min_Max{0, claydo.MAX_FLOAT}}}
    ...
    if arguments.elapsed_time < arguments.duration {
        ...
        if allProperties || .BOUNDING_BOX in arguments.properties {
            arguments.current.bounding_box = {
                x = rl.Lerp(arguments.initial.bounding_box.x, arguments.target.bounding_box.x, lerpAmount),
                y = rl.Lerp(arguments.initial.bounding_box.y, arguments.target.bounding_box.y, lerpAmount),
                width = rl.Lerp(arguments.initial.bounding_box.width, arguments.target.bounding_box.width, lerpAmount),
                height = rl.Lerp(arguments.initial.bounding_box.height, arguments.target.bounding_box.height, lerpAmount),
            }
        } else {
        ...
        if allProperties || .OVERLAY_COLOR in arguments.properties {
            arguments.current.overlay_color = {
                    rl.Lerp(arguments.initial.overlay_color.r, arguments.target.overlay_color.r, lerpAmount),
                    rl.Lerp(arguments.initial.overlay_color.g, arguments.target.overlay_color.g, lerpAmount),
                    rl.Lerp(arguments.initial.overlay_color.b, arguments.target.overlay_color.b, lerpAmount),
                    rl.Lerp(arguments.initial.overlay_color.a, arguments.target.overlay_color.a, lerpAmount),
            };
        } else {
...
HandleRandomiseButtonInteraction :: proc(elementId: claydo.Element_ID, cursorData: claydo.Cursor_Data, userData: rawptr) {
    if (cursorData.state == .PRESSED_THIS_FRAME) {
        shuffle(colors[:], cellCount);
    }
}

HandlePinkButtonInteraction :: proc(elementId: claydo.Element_ID, cursorData: claydo.Cursor_Data, userData: rawptr) {
    if (cursorData.state == .PRESSED_THIS_FRAME) {
...
HandleNewButtonInteraction :: proc(elementId: claydo.Element_ID, cursorData: claydo.Cursor_Data, userData: rawptr)
{
    if (cursorData.state == .PRESSED_THIS_FRAME) {
...
HandleBlueButtonInteraction :: proc(elementId: claydo.Element_ID, cursorData: claydo.Cursor_Data, userData: rawptr)
{
    if cursorData.state == .PRESSED_THIS_FRAME {
    for i := 0; i < cellCount; i += 1 {
            index := colors[i].id;
...
CreateLayout :: proc() -> []claydo.Render_Command
{
        ...
        {claydo.ui(claydo.id("OuterContainer"))({
                layout = {
                        ...
                        child_gap = 12 },
                color = cWHITE })
                {claydo.ui()({layout = {sizing = {claydo.sizing_grow(), claydo.sizing_fixed(60) }, padding = {left = 16}, child_gap = 16, child_alignment = {y = .CENTER}}, corner_radius = { 12, 12, 12, 12 }, color = ({174, 143, 204, 255 } / 255) })
	        ...
	        for i := 0; i < 5; i += 1 {
	                {claydo.ui(claydo.idi("row", u32(i)))({layout = {child_gap = 12, sizing = GG }})
	                        for j := 0; j < 6; j += 1 {
	                                ...
	                                darker := claydo.Color{ colors[index].color.r * 0.9, colors[index].color.g * 0.9, colors[index].color.b * 0.9, 1 }
						...
						if claydo.clicked() {
							...
							cellCount = max(cellCount - 1, 0)
	                                        ...
	                                        claydo.text(colors[index].stringId, {
	                                                font_size = 32,
...
UpdateDrawFrame :: proc()
{
    ...
    if (rl.IsMouseButtonDown(.LEFT) && !scrollbarData.mouseDown && claydo.cursor_over(claydo.hash_string("ScrollBar", 0))) {
        scrollContainerData := claydo.get_scroll_container_data(claydo.hash_string("MainContent", 0));
        ...
        if (scrollContainerData.content_dimensions.y > 0) {
            ...
            if (scrollContainerData.config.vertical) {
                scrollContainerData.scroll_position.y = scrollbarData.positionOrigin.y + (scrollbarData.clickOrigin.y - mousePosition.y) * ratio.y;
            }
            if (scrollContainerData.config.horizontal) {
                scrollContainerData.scroll_position.x = scrollbarData.positionOrigin.x + (scrollbarData.clickOrigin.x - mousePosition.x) * ratio.x;
            }
        }
    }
...
HandleClayErrors :: proc(errorData: claydo.Error_Data) {
    ...
    if (errorData.type == .ELEMENTS_CAPACITY_EXCEEDED) {
        ...
        reinitializeClay = true;
        claydo.set_max_measure_text_cache_word_count(claydo.get_max_measure_text_cache_word_count() * 2);
    }
}
...
main :: proc()
{
    ...
    claydo.initialize(arena, {cast(f32)rl.GetScreenWidth(), cast(f32)rl.GetScreenHeight()}, { err_proc = HandleClayErrors })
    claydo.set_measure_text_procedure(rr.measure_text, nil)
    rr.raylib_initialize(1024, 768, "Clay - Raylib Renderer Example", {.VSYNC_HINT, .WINDOW_RESIZABLE, .MSAA_4X_HINT});
